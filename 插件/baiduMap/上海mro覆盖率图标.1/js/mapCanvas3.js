...
arrs:[{
  ...
  showTextArr:array 显示字体   //该项很耗性能 如果不添加页面会更加流程
}]
...
(function(win) {
    ...
    ComplexCustomOverlay.prototype = $.extend({}, new BMap.Overlay(), {
        ...
        draw: function(eventp, activep, callback) {
            ...
            for (let index = 0, len = $this.data.length; index < len; index++) {
                ...
                if (viewBound.containsPoint(point)) {
                    ...
                    //sectorS
                    if (val.shape == "sectorS1" && zoom >= $this.opts.showShapeZoom.sectorS1) {
                        $this.drawSectorS1(context, val, distance, xyCanvas)
                    }
                    //circle
                    ...
                    //circleS
                    if (val.shape == "circleS1" && zoom >= $this.opts.showShapeZoom.circleS1) {
                        $this.drawCircleS1(context, val, distance, xyCanvas)
                    }
                    //circleO
        ...
        drawCircleS1: function(context, val, distance, xyCanvas, zoom) {
            //起始一条路径，或重置当前路径
            context.arc(xyCanvas.x, xyCanvas.y, distance / 2, 0, 2 * Math.PI, false);
            grd1 = context.createRadialGradient(xyCanvas.x, xyCanvas.y, 0, xyCanvas.x, xyCanvas.y,
                distance * .5);
                
            grd1.addColorStop(0.1, val.color);
            grd1.addColorStop(0.2, val.color);
            grd1.addColorStop(0.3, val.color.colorRgba(8));
            grd1.addColorStop(0.4, val.color.colorRgba(7));
            grd1.addColorStop(0.6, val.color.colorRgba(4));
            grd1.addColorStop(0.8, val.color.colorRgba(1)); 
            /*   grd1.addColorStop(0.4, val.color.colorRgba(9));
             grd1.addColorStop(0.60, val.color.colorRgba(8));
             grd1.addColorStop(0.65, val.color.colorRgba(7));
             grd1.addColorStop(0.7, val.color.colorRgba(6));
             grd1.addColorStop(0.75, val.color.colorRgba(5));
             grd1.addColorStop(0.80, val.color.colorRgba(4));
             grd1.addColorStop(0.85, val.color.colorRgba(3));
             grd1.addColorStop(0.90, val.color.colorRgba(2));
             grd1.addColorStop(0.95, val.color.colorRgba(1)); */
            grd1.addColorStop(1, 'rgba(255, 255, 255, 0)');
            //填充为黄色
            context.fillStyle = grd1;
            //填充当前绘图（路径）
            context.fill();
            //val.color.colorRgba(5)
        },
        ...
        drawSectorS1: function(context, val, distance, xyCanvas) {
            let grd1, grd2;
            //扇形
            grd1 = context.createRadialGradient(xyCanvas.x, xyCanvas.y, 0, xyCanvas.x, xyCanvas.y,
                distance);
            grd1.addColorStop(0.2, val.color);
            grd1.addColorStop(0.3, val.color);
            grd1.addColorStop(0.5, val.color.colorRgba(7));
            grd1.addColorStop(0.7, val.color.colorRgba(4));
            grd1.addColorStop(0.9, val.color.colorRgba(1));
           /* grd1.addColorStop(0.4, val.color.colorRgba(9));
            grd1.addColorStop(0.60, val.color.colorRgba(8));
            grd1.addColorStop(0.65, val.color.colorRgba(7));
            grd1.addColorStop(0.7, val.color.colorRgba(6));
            ...
            grd1.addColorStop(0.95, val.color.colorRgba(1)); */
            grd1.addColorStop(1, "rgba(255, 255, 255, 0)");
            context.sector(xyCanvas.x, xyCanvas.y, distance,
                val.dir - 90 - (120 / 2),
                val.dir - 90 + (120 / 2))
            context.fillStyle = grd1;
            context.fill();
